...
def analyze_comment(comment):
    ...
    result['POS_EMOS'] = comment.count('EMO_POS')
    result['NEG_EMOS'] = comment.count('EMO_NEG')
    words = comment.split()
...
def get_bigrams(comment_words):
    bigrams = []
    num_words = len(comment_words)
    for i in range(num_words - 1):
        bigrams.append((comment_words[i], comment_words[i + 1]))
    return bigrams
...
if __name__ == '__main__':
    if len(sys.argv) != 2: 
        exit()
    num_comments, num_pos_comments, num_neg_comments = 0, 0, 0
    ...
    with open(sys.argv[1], 'r') as csv:
        ...
        for i, line in enumerate(lines):
            t_id, if_pos, comment = line.strip().split(',')
            if_pos = int(if_pos)
            if if_pos:
                num_pos_comments += 1
            else:
                num_neg_comments += 1
            result, words, bigrams = analyze_comment(comment)
